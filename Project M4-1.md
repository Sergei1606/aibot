...
### 1. Введение
Вас взяли работать в стартап, который хочет автоматизировать новостной Telegram-канал на новом уровне. Ваша задача — сделать сервис, который не просто парсит новости, но использует ИИ для генерации ярких, лаконичных и интересных постов на основе собранных материалов. Публикация должна идти в Telegram-канал по расписанию, с возможностью ручного управления и мониторинга через API.
...
- README.md с описанием, структурой, инструкцией по запуску, примером API-запросов.
- (По желанию) — docker-compose файл, скринкаст работы или демонстрационный Telegram-канал.
- Ссылка на репозиторий + ФИО и учебную группу прислать преподавателю.
- Чек-лист с отметками по реализации (можно приложить отдельно).